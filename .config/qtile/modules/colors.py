monochrome = [
    ["#101010", "#101010"],  # 0 BACKGROUND
    ["#575757", "#575757"],  # 1 BORDER NORMAL  INACTIVE
    ["#3f3f3f", "#3f3f3f"],  # 2 OTHER SCREEN BORDER
    ["#6f6f6f", "#6f6f6f"],  # 3 HIGHLIGHT-COLOR
    ["#9f9f9f", "#9f9f9f"],  # 4 BORDER FOCUS
    ["#b9b9b9", "#b9b9b9"]   # 5 FOREGROUND  ACTIVE  CURRENT BORDER 
]


tokyonight = [
    ["#1a1b26", "#1a1b26"], # 0 dark background
    ["#414868", "#414868"], # 1 grey
    ["#c0caf5", "#c0caf5"], # 2 light active
    ["#a9b1d6", "#a9b1d6"], # 3 text foreground
    ["#7aa2f7", "#7aa2f7"], # 4 focus
    ["#787c99", "#787c99"], # 5 incative
    ["#f7768e", "#f7768e"], # 6 urgent
    ["#bb9af7", "#bb9af7"], # 7 color1
    ["#9ece6a", "#9ece6a"], # 8 color2
]
